...
class Program
{
    ...
    static void Main()
    {
        ...
        var task1 = debtors.Where(db => db.Email.EndsWith("rhyta.com") || db.Email.EndsWith("dayrep.com")).ToList();

        task1.ForEach(db => Console.WriteLine(db));

        ////2) Yashi 26 - dan 36 - ya qeder olan borclulari cixartmag

        var task2 = debtors.Where(db => DateTime.Now.Year - db.BirthDay.Year >= 26 && DateTime.Now.Year - db.BirthDay.Year <= 36).ToList();

        task2.ForEach(db => Console.WriteLine(db.BirthDay));

        //3) Borcu 5000 - den cox olmayan borclularic cixartmag

        var task3 = debtors.Where(db => db.Debt <= 5000).ToList();

        task3.ForEach(db => Console.WriteLine(db.Debt));

        //4) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq

        var task4 = debtors.Where(db => db.FullName.Length >= 18 && db.Phone.Count(c => c == '7') >= 2).ToList();

        task4.ForEach(db => Console.WriteLine(db.Phone + db.FullName));

        //5) Qishda anadan olan borclulari cixardmaq

        var task5 = debtors.Where(db => db.BirthDay.Month == 12 || db.BirthDay.Month == 1 || db.BirthDay.Month == 2).ToList();

        task5.ForEach(db => Console.WriteLine(db.BirthDay));
        ...
        var task6 = debtors.Where(db => db.Debt > debtors.Average(db => db.Debt)).OrderByDescending(db => db.FullName.Split()[2]).OrderByDescending(db => db.Debt).ToList();


        task6.ForEach(db => Console.WriteLine(db));


        //7) Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq

        var task7 = debtors.Where(db => !db.Phone.Contains('8')).ToList();

        task7.ForEach(db => Console.WriteLine(db.FullName.Split()[2] + " " + db.BirthDay + " " + db.Debt));
        ...
        task8.ForEach(db => Console.WriteLine(db));

        //9)borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq

        var task9 = debtors.Where(db => db.Debt == debtors.Max(db => db.Debt)).ToList();

        task9.ForEach(db => Console.WriteLine(db.BirthDay.Year));

        //10)Borcu en boyuk olan 5 borclunun siyahisini cixartmaq

        var task10 = debtors.OrderByDescending(db => db.Debt).ToList();

        for (int i = 0; i < 5; i++)
        {
            Console.WriteLine(task10[i]);
        }

        //11)Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq

        Console.WriteLine(debtors.Sum(db => db.Debt));


        //12)2ci dunya muharibesin gormush borclularin siyahisi cixartmaq

        var task12 = debtors.Where(db => db.BirthDay.Year <= 1945).ToList();

        task12.ForEach(db => Console.WriteLine(db));
        ...
        var task15 = debtors.Where(db => db.FullName.ToLower().Contains("smile")).ToList();

        task15.ForEach(db => Console.WriteLine(db.FullName));
    }
}